...
/// </remarks>
///--------------------------------------------------------------------
UInt16 GetIU16FrombyteArray(byte* data, int location)
{
    return (UInt16)((data[location]) + (data[location + 1] << 8));
}
...
void PutIU16TobyteArray(UInt16 number, byte* data, int location)
{
    data[location + 1] = (byte)((number >> 8) & 0xFF);
    data[location] = (byte)(number & 0xFF);
}
