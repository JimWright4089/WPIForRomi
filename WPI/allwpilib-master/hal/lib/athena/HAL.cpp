...
//  printf("HAL_BaseInitialize A\n");
  static std::atomic_bool initialized{false};
//  printf("HAL_BaseInitialize B\n");
  static priority_mutex initializeMutex;
  printf("HAL_BaseInitialize C\n");
  // Initial check, as if it's true initialization has finished
  if (initialized) return;
  printf("HAL_BaseInitialize D\n");

  std::lock_guard<priority_mutex> lock(initializeMutex);
  printf("HAL_BaseInitialize E\n");
  // Second check in case another thread was waiting
  if (initialized) return;
  // image 4; Fixes errors caused by multiple processes. Talk to NI about this
  printf("HAL_BaseInitialize F\n");
  nFPGA::nRoboRIO_FPGANamespace::g_currentTargetClass =
      nLoadOut::kTargetClass_RoboRIO;

  printf("HAL_BaseInitialize G\n");
  global.reset(tGlobal::create(status));
  printf("HAL_BaseInitialize H\n");
  watchdog.reset(tSysWatchdog::create(status));
  printf("HAL_BaseInitialize I\n");
  initialized = true;
}
...
int32_t HAL_Initialize(int32_t mode) {
  printf("HAL_Initialize A\n");
  setlinebuf(stdin);
  setlinebuf(stdout);

  printf("HAL_Initialize B\n");
  prctl(PR_SET_PDEATHSIG, SIGTERM);

  printf("HAL_Initialize C\n");
  FRC_NetworkCommunication_Reserve(nullptr);

  printf("HAL_Initialize D\n");
  std::atexit([]() {
    // Unregister our new data condition variable.
    setNewDataSem(nullptr);
  });

  printf("HAL_Initialize E\n");
  int32_t status = 0;
  HAL_BaseInitialize(&status);

  printf("HAL_Initialize F\n");
  if (!rolloverNotifier)
  ...
  printf("HAL_Initialize J\n");
  HAL_InitializeDriverStation();
  printf("HAL_Initialize K\n");
