...
class Message
{
    public:
        ...
        int GetMessageID();
        void GetSendData(byte* returnBytes, byte returnBytesLength);
...
class DS_Status_Message : public Message
{
        ...
        static const int ALL_BYTE_LOC = 5;
        static const int TIME_BYTE_LOC = 10;

        DS_Status_Message();
        DS_Status_Message(byte* data, byte dataLen);
        void SetStatus(byte status);
        void SetAll(byte all);
...
class RC_Status_Message : public Message
{
        ...
        RC_Status_Message();
        RC_Status_Message(byte* data, byte dataLen);
        void SetStatus(byte status);
        void SetTime(UInt32 theTime);
        UInt32 GetTime();
        byte GetStatus();
...
public:
    Controller_Message();
    Controller_Message(byte* data, byte dataLen);
